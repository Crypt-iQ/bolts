...
attached to either side for fee-bumping. Note that the kickoff tx and commit tx break CPFP
carve-out even with the addition of anchors at the kickoff level. The semantics of carve-out
are such that the "carve-out" transaction must only have one ancestor. It is therefore not
possible to use the commitment transaction's anchor as a carve-out since it has two ancestors:
